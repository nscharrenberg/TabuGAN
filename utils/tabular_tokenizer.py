...
class TabularTokenizer(object):
    ...
    def __init__(self, coder_file,
        ...
        if special_tokens is None:
            special_tokens = ['<end>', '\n']
        elif '<end>' not in special_tokens:
            raise ValueError(
                '<end> token must be present as a special token')
        ...
        self.eod_id = self.special_tokens['<end>']
    ...
    def tokenize_str(self, text: str) -> List[str]:
        ...
        endoftext = '<end>'
        tokens = []
        # handle case where endoftext token is in middle of string (2+ documents)
        for doc in text.split(endoftext):
            ...
            for row in rows:
                ...
                if not used_eot:
                    # split into fields
                    tokens.extend(
                        [endoftext] + ''.join(row.split(endoftext)).split(self.delimiter))
                    used_eot = True
                else:
                    tokens.extend(''.join(row.split(endoftext)).split(
                        self.delimiter))  # split into fields
                tokens.append('\n')
        return tokens
        ...
        for i in ids:
            if i in self.special_tokens_decoder:
                ...
                if not keyerror and (len(token_ids) == sizes[column_index]):
                    try:
                        tokens.append(
                            self.code_column.decode(column, token_ids))
