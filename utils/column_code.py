...
class FloatTokenizer(Tokenizer):
    ...
    def __init__(self, col_name: str, data_series: DataFrame, start_id: int, transform='yeo-johnson'):
        ...
        if transform == 'yeo-johnson':
            ...
            raise ValueError(
                'Supported data transformations are "yeo-johnson", "quantile", and "robust", and "log"')
        ...
        if self.scaler_name in ['yeo-johnson', 'quantile', 'robust']:
            ...
            values = np.unique(self.scaler.fit_transform(
                data_series.values)).astype(float)
        else:
        ...
        integral_digits = int(
            np.log10(np.abs(values[~np.isnan(values)]).max()) + 1)
    ...
    def log_vocab(self, data_series):
        ...
        self.code_len = len(str(significant_val.max())) if len(
            str(significant_val.max())) > self.code_len else self.code_len
    ...
    def encode(self, item: str) -> List[int]:
        if self.scaler_name == 'log':
            ...
            if item == 'nan':
                ...
                for i in reversed(range(self.code_len)):
                    if valstr[self.code_len - i - 1] not in self.digits_item_to_id[i]:
                        self.digits_item_to_id[i][valstr[self.code_len -
                                                         i - 1]] = self.end_id
                ...
                valstr = str(
                    abs((values * 10 ** self.extra_digits).astype(int)))[:self.code_len]
            ...
            for i in reversed(range(self.code_len)):
                codes.append(
                    self.digits_item_to_id[i][valstr[self.code_len - i - 1]])
            return codes
    ...
    def decode(self, ids: List[int]) -> str:
        if self.scaler_name == 'log':
            ...
            for i in reversed(range(self.code_len)):
                items.append(
                    self.digits_id_to_item[i][ids[self.code_len - i - 1]])
            v = int("".join(items))
...
class CategoricalTokenizer(Tokenizer):
    ...
    def __init__(self, col_name: str, data_series: Series, start_id: int):
        ...
        self.item_to_id = pd.Series(
            data=uniq_items.index, index=uniq_items.values).to_dict()
